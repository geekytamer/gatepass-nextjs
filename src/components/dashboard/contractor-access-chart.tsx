...
import { collection, onSnapshot, query, where, Query, getDocs } from 'firebase/firestore';
...
export function ContractorAccessChart({ siteId, companyId }: ContractorAccessChartProps) {
  ...
  useEffect(() => {
    ...
    const setupListener = async () => {
        ...
        if (companyId !== 'all') {
            // Need to check if company is operator or contractor
            const isOperator = (await getDocs(query(collection(firestore, 'operators'), where('__name__', '==', companyId)))).size > 0;
            if (isOperator) {
                requestsQuery = query(requestsQuery, where('operatorId', '==', companyId));
            } else {
                 requestsQuery = query(requestsQuery, where('contractorId', '==', companyId));
            }
        }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
            Distribution of approved requests across contractors for the selected filter.
        </CardDescription>
      </CardHeader>
        ...
        {loading ? (
           <div className="h-[250px] w-[250px]">
            ...
            <div className="h-[250px] flex items-center justify-center text-muted-foreground">
                No approved requests found for this selection.
            </div>
        )}
      </CardContent>
    </Card>
